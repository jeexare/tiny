document.addEventListener("DOMContentLoaded", function () {
  ...
  // Contador
	const hasCounters = document.querySelectorAll(".hascount");
	hasCounters.forEach((hasCounter) => {
		const countmeElement = hasCounter.querySelector(".countme");
		if (!countmeElement) return;
		const counterElement = hasCounter.querySelector(".wordCount");
		...
		// Load the content of .countme file (you may use AJAX, Fetch, or any other method here)
		const content = countmeElement.textContent;
		if (!content) return;
...
// Lanzada de dados
const dadosLanzadas = Array.from(document.querySelectorAll('.postbody')).filter(el => el.textContent.includes('ha efectuado la acción siguiente: Lanzada de dados'));
for (dadoLanzada of dadosLanzadas) {  
  // Crear div lanzada-dados
  const newDiv = document.createElement('div');
  newDiv.classList.add('lanzada-dados');
  newDiv.classList.add('hidden-pp');
  
  while (dadoLanzada.firstChild)
    newDiv.appendChild(dadoLanzada.firstChild);
  dadoLanzada.appendChild(newDiv);
  
  // Quitar los saltos de linea
    const brElements = newDiv.querySelectorAll('br');
    brElements.forEach(function(br) {
      br.remove();
    });
  
  
   // Rodear cada texto con un span
...
// Modificar texto comienzo
const childElements = newDiv.querySelectorAll('*');
for (let i = 0; i < childElements.length; i++) {
  if (i === 0) 
  childElements[i].remove();
    if (i === 2) 
  childElements[i].textContent = " deja que el destino decida";
    if (i ===3) 
  childElements[i].remove();
   console.log('Child Element:', childElements[i].textContent);
}
  
  
  // Quitar lineas y comas
  const line = '--------------------------------';
    const comma = ', ';

const spans = newDiv.querySelectorAll('span');
spans.forEach(function(span) {
  if (span.textContent.includes(line)) {
    span.textContent = span.textContent.replace(line, '');
  }

  if (span.textContent.includes(comma)) {
    span.textContent = span.textContent.replace(comma, '');
  }
});

  // Despues del HR
  
  const hrElements = newDiv.querySelectorAll('hr');
    hrElements.forEach(function(hr) {
      const nextElements = getNextSiblings(hr);
      const tiradaDiv = document.createElement('div');
      tiradaDiv.className = 'tirada';
      nextElements.forEach(function(nextElement) {
        tiradaDiv.appendChild(nextElement);
      });
      hr.insertAdjacentElement('afterend', tiradaDiv);
    });
 

  // Remove the empty <span></span> elements
const emptySpans = newDiv.querySelectorAll('span:empty');
emptySpans.forEach(function (emptySpan) {
  emptySpan.remove();
});

  // Clases a los span de tirada
const tiradaSpans = newDiv.querySelectorAll('.tirada span');
tiradaSpans.forEach(function (span) {
  const prevSpanText = span.textContent;
  const start = prevSpanText.indexOf("'") + 1;
  const end = prevSpanText.indexOf("'", start);
  const diceName = prevSpanText.substring(start, end).replace(/\s+/g, '-').toLowerCase();
  span.textContent = diceName;
  
  // Add a class to the current 'span' element based on the extracted diceName
  span.classList.add(`dado-${diceName}`);

  const elementsToAdd = [];
  let nextElement = span.nextElementSibling;

  // Collect all the siblings that come after the current 'span' element and before the next 'span' element
  while (nextElement && nextElement.tagName !== 'SPAN') {
    elementsToAdd.push(nextElement);
    nextElement = nextElement.nextElementSibling;
  }

  const resultadoDiv = document.createElement('div');
  resultadoDiv.className = `resultado-${diceName}`;

  // Move the collected siblings into the new 'resultadoDiv'
  elementsToAdd.forEach(function (element) {
    resultadoDiv.appendChild(element);
  });

  // Insert the 'resultadoDiv' right after the current 'span' element
  span.insertAdjacentElement('afterend', resultadoDiv);
  
       const strongElements = resultadoDiv.querySelectorAll('strong');
      strongElements.forEach(function(strong) {
        const result = strong.textContent;
        strong.classList.add(`${diceName}-res-${result}`);
      });
});
