document.addEventListener('DOMContentLoaded', function() {
    ...
    postLabels.forEach(function(label) {
      ...
      var labelText = label.textContent.trim().split(' ')[0].toLowerCase().replace(/'/g, '').replace(/\(|\)/g, '');
      parentPostfield.classList.add(labelText);
    });
  
    // Cronología
    const currentDate = document.querySelector('.th-crono span');
    const [monthString, year] = currentDate.textContent.match(/(\w+) (\d+)/).slice(1);
    ...
    colorProperties.forEach(function(property) {
      var colorValue = getComputedStyle(document.documentElement).getPropertyValue(property).trim();
    ...
    postRows.forEach(function(postRow) {
      ...
      for (var i = 0; i < codeElements.length; i++) {
        ...
        var encodedContent = codeContent.replace(/</g, '&lt;').replace(/>/g, '&gt;');
      ...
      var imgBaul = postRow.querySelector('img[src*="https://thumbs2.imgbox.com/c4/76/vzQNtmuv_t.png"]');
      if (imgBaul) {
      ...
      if (modoPNJ) {
        ...
        if (nombreNuevo) {
          var nombreContainer = postRow.querySelector('.post-username a span strong');
                  if (nombreContainer)
        ...
        if (rangoNuevo) {
          var rangoContainer = postRow.querySelector('.ph-right span:not([class])');
      ...
      var postuserElement = postRow.querySelector(".pp-headermain .post-username");
    ...
    var codeBoxes = document.querySelectorAll('dl.codebox:not(.spoiler,.hidecode) > dd.code, dl.codebox:not(.spoiler,.hidecode) > dd > code');
